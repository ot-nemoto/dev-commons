...
> **⚠️ このリポジトリが外部 API・DB・認証のいずれも持たない場合、以下のルールは適用対象外（無視してよい）。** 該当の有無は `.claude/project.md` のドキュメント採否・実コード（`app/api/`・DB・認証の利用）で判断する。
>
...
該当する実装がある場合にのみ作成を必須とする（採否は各リポジトリの `.claude/project.md`「ドキュメント採否」を正とする）。source of truth 原則に従い、各 doc は意図・契約・規約を扱う（コードが正の内容は書かない）。
